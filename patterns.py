...
SQL_Patterns=r"""   patterns = (\"|')?(SQL|sql|Sql)?_?(ADMIN|admin|Admin)?_?(CREDENTIAL|credential|Credential)?_?(PWD|pwd|Pwd)?_?(PASSWORD|password|Password)(\"|')?\\s*(:|=>|=)\\s*(\"|')?[A-zA-Za-z0-9!@$%&*?"#"\\^=]{10,15}(\"|')?"""
